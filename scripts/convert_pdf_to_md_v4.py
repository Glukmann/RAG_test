...
# -*- coding: utf-8 -*-
"""
PDF → Markdown конвертер для RSL документации (v4).
Ключевые улучшения:
- Удаление строк оглавления (точки + номер страницы) и пустых заголовков.
- Улучшенное определение границ оглавления (выбор правильного Введение).
...
def process_rslprc(text: str, title: str) -> str:
    ...
    for i, m in enumerate(matches):
        ...
        if is_toc_entry(body):
            continue
        if kind in ('Процедура', 'Функция', 'Метод') and not is_valid_proc_name(name, body):
            continue
