...
языка RSL, которые используются при создании макромодулей, входящих в АС RS-Retail
V.6, и при написании пользователем собственных макропрограмм. 
...
V.6, RS-Banking V.6, RS-Loans V.6, RS-FinMarkets, приведено в соответствующих
Руководствах к этим программным комплексам.
...
которых соответствует названию модуля. Руководство содержит примеры, иллюстрирующие
использование интерфейсов при написании программ.
Внимание!
...
В настоящем документе содержатся ссылки на следующие Руководства:
· Справочное 
руководство 
"Валютно-обменные 
операции" 
(файл
Books\Retail\DepExchange.pdf).
· Руководство 
администратора 
"Система 
управления 
...
· Руководство пользователя "Сейфовые ячейки" (файл Books\Retail\DepCells.pdf).
...
Глобальные переменные
Глобальные переменные используются в макросах и доступны в любой из подсистем АС
RS-Retail V.6.
Переменные для обслуживания
карточных счетов
Глобальные переменные, перечисленные ниже, используются в макросах обслуживания
карточных объектов (счета, карточки, связи карточки и счета, транзакции, платежные
системы). Все макросы вызываются вне транзакций.
В таблице содержится информация о таблицах, для обработки которых можно вызывать
макросы (базовые таблицы):
Номер
таблицы
Имя таблицы
Комментарий
dscAcc_dbt
CardAccount (карточные счета)
dscCard_dbt
Card (карточки)
dscLink_dbt
Link (связи счетов и карточек)
dscPSSch_dbt
PaymentSuystemShem (схемы расчета)
dscAuth_dbt
Transactions (транзакции)
dscDTran_dbt
DocumentOfTransaction (документы транзакции)
dscCodif_dbt
Codificator (кодификатор)
dscLGrPr_dbt
LinkGroupProperty (группы и свойства)
dscMail_dbt
MailSession (сеансы связей)
dscMailIt_dbt
MailSessionItem (расшифровки сеансов связей)
ALG_RET_OP
ALG_RET_OP (ret_op) – определитель типа RECORD, который при работе в контексте
операции (в том числе и нефинансовой) заполняется данными записи таблицы
операций dret_op_dbt, а именно значениями следующих полей: t_Branch, t_IsCur,
t_Date, 
t_Operation, 
t_SubOp, 
t_Brigade, 
...
поле 
Flags
устанавливается ROF_NINFINANCIAL=1.
Глобальные переменные
ALG_SECONDDO
ALG_SECONDDO – уточнение события. Переменная может принимать следующие
значения:
· Для уточнения события SAVE:
· 1 – сохранение новой записи (Binsert);
· !1 – сохранение записи после корректировки.
· Для уточнения события OPCL:
· 1 – открытие объекта;
· 2 – закрытие объекта.
ALG_STAT
ALG_STAT – код возврата макроса. Переменная может принимать следующие значения:
НЕТ_ОШИБКИ = 0;
ЕСТЬ_ОШИБКА = 1664.
Если макрос возвратил ненулевой код, то следующее плановое действие
выполнено не будет. 
В случае завершения макроса вызова из формы (SCPSMAC_PANEL) с
ненулевым кодом, считается, что ввод данных осуществлен некорректно, и будет
восстановлена прежняя запись (до вызова макроса из формы).
ALG_VARPARTSIZE
ALG_VARPARTSIZE – длина переменной части таблицы. Эта глобальная переменная
особенно важна в контексте макросов инициализации, так как используется для
определения размера структуры введенных данных.
ALG_WHATDO
ALG_WHATDO – номер события, при котором вызван макрос. Переменная может
...
Инициализация
нового объекта
Вызывается при создании нового
объекта 
после 
инициализации
Глобальные переменные
...
корректировки
объекта
Вызывается после выполнения всех
действий, 
необходимых 
для
обращения к форме корректировки,
но до отображения формы.
SCPSMAC_PANEL
= 3
Вызов 
из
...
формы 
ввода/
корректировки объекта по клавише
Alt+M, 
перед 
вызовом 
макроса
сохраняются внесенные в форме
изменения.
SCPSMAC_SAVE
= 4
Сохранение
...
и
корректировки)
Вызывается после осуществления
всех инициализаций и проверок до
транзакции. 
Макросу 
передается
целочисленный параметр: 1 – новая
запись (ввод, Binsert), в остальных
случаях – корректировка.
SCPSMAC_DELETE
= 5
Удаление
объекта
...
перед оплатой транзакции или перед
аннулированием транзакции.
Связь между базовыми таблицами и событиями описана ниже:
Таблица
INIT
ENTER
PANEL
...
–
–
CardsLimitOverRunDate
CardsLimitOverRunDate – дата операционного дня, в котором превышено лицензионное
ограничение на количество открытых пластиковых карт. По умолчанию
установлено 
значение 
BDATE_ZERO, 
означающее, 
что 
лицензионное
ограничение не превышено и функциональность по пластиковым картам доступна
в полном объеме.
SC_ACC
SC_ACC – определитель типа RECORD текущей записи таблицы карточных счетов,
описанной во внутреннем словаре как dScAcc_dbt.
SC_CARD
SC_CARD – определитель типа RECORD текущей записи таблицы карточек, описанной во
внутреннем словаре как dScCard_dbt.
SC_CODIF
SC_CODIF – определитель типа RECORD текущей записи таблицы кодификатора,
описанной во внутреннем словаре как dScCodif_dbt.
SC_DTRAN
SC_DTRAN – определитель типа RECORD текущей записи таблицы документов
транзакции, описанной во внутреннем словаре как dScDTran_dbt.
SC_LGP
SC_LGP – определитель типа RECORD текущей записи таблицы групп, описанной во
внутреннем словаре как dScLGrPr_dbt.
Глобальные переменные
SC_LINK
SC_LINK – определитель типа RECORD текущей записи таблицы связей "карточка-счет",
описанной во внутреннем словаре как dScLink_dbt.
SC_MAIL
SC_MAIL – определитель типа RECORD текущей записи таблицы сеансов связей,
описанной во внутреннем словаре как dScMail_dbt.
SC_MAILIT
SC_MAILIT – определитель типа RECORD текущей записи таблицы расшифровки сеансов
связей, описанной во внутреннем словаре как dScMailIt_dbt.
SC_PS
SC_PS – определитель типа RECORD текущей записи таблицы платежных систем,
описанной во внутреннем словаре как dScPos_dbt.
SC_PSS
SC_PSS – определитель типа RECORD текущей записи таблицы схем расчета, описанной
во внутреннем словаре как dScPSSch_dbt.
SC_TRAN
SC_TRAN – определитель типа RECORD текущей записи таблицы транзакций, описанной
во внутреннем словаре как dScAuth_dbt.
Пример.
record SC_TRAN ("scTran.dbt", "spcard.def");
Прочие переменные
ALG_DEPOSITOR
ALG_DEPOSITOR – определитель типа RECORD текущей записи таблицы, описанной во
внутреннем словаре как ddepositr_dbt, с которой осуществляется работа
программы. Значение переменной передается в макрос для получения
дополнительной информации и корректировки текущей записи таблицы.
Глобальные переменные
Переменные ALG_DEPOSITOR и ALG_SBDEPDOC
 используются в макросах,
с помощью которых реализуются шаги операции.
Пример.
record ALG_DEPOSITOR (depositr);
ALG_PARAM
ALG_PARAM – определитель типа RECORD записи таблицы параметров выполняемого
шага операции.
Пример.
record ALG_PARAM ("sb_algop.dbt");
ALG_RESULT
ALG_RESULT – код возврата макроса, определяющий выполнение следующих шагов
операции или операции в целом. Эта переменная обязательно используется в
макросах, с помощью которых выполняются шаги операций по счетам вкладчиков
в подсистеме "Универсальный фронт-офис". Переменная может принимать
следующие значения:
Значение
Комментарий
OK_RES
= 0
Макрос завершен успешно, работа будет продолжена
SKIP_RES
= 1
Макрос завершен успешно, но следует отказаться от
дальнейшего выполнения текущего шага операции и
перейти к выполнению следующего шага операции.
END_RES
= 2
Завершение 
проводки 
...
которого 
были
выполнены все необходимые действия по операции.
ERR_RES
= -1
Аварийно завершение проводки (операции). Макрос
завершен 
с 
ошибкой, 
проводка 
...
не
выполнена.
Все коды возврата объявлены в макросе alg_vars.mac.
ALG_SBDEPDOC
ALG_SBDEPDOC – определитель типа RECORD текущей записи таблицы, описанной во
внутреннем 
словаре 
как 
dsbdepdoc_dbt 
...
с 
которой
осуществляется работа программы. Значение переменной передается в макрос
для получения дополнительной информации и корректировки текущей записи
таблицы.
Переменные ALG_DEPOSITOR
 и ALG_SBDEPDOC используются в макросах,
с помощью которых реализуются шаги операции.
Глобальные переменные
Пример.
record ALG_SBDEPDOC (sbdepdoc);
ALG_UPDATE
ALG_UPDATE – битовый флаг, определяющий переменную, значение которой будет
обновлено в базе данных в случае изменения соответствующей записи макросом.
При компиляции текста программы целочисленное значение параметра будет
переведено 
в 
двоичную 
систему 
...
в 
виде
последовательности символов "0" и "1". Глобальная переменная может
принимать следующие значения:
Значение
Битовый
флаг
...
UPDATE_NOT
= 0
Значения переменных не обновляются.
UPDATE_DEP
= 2
Будет 
обновлено 
...
переменной
ALG_SBDEPDOC
 – запись о документе в
таблице dsbdepdoc_dbt или dfcdepdoc_dbt.
UPDATE_
COMCLNT
= 8
Будет 
...
значение 
переменной
ALG_COMDEPCLIENT – запись о клиенте.
UPDATE_ALL
= 1
Будут обновлены значения переменных
ALG_DEPOSITOR
 и ALG_SBDEPDOC
.
CUR_SBDEPDOC
CUR_SBDEPDOC – определитель типа RECORD записи таблицы, описанной во
внутреннем 
словаре 
как 
dsbdepdoc_dbt 
...
переменная 
определяет
документ, который был отмечен курсором перед вызовом операции. Отличие этой
переменной от ALG_SBDEPDOC
 заключается в невозможности корректировки
полей документа. Значение переменной передается в макрос для получения
дополнительной информации.
Пример.
record CUR_SBDEPDOC (sbdepdoc);
Глобальные переменные
DEP_ACC_REC
DEP_ACC_REC – определитель типа RECORD текущей записи таблицы счетов. Значение
переменной передается в макрос определения суммы корректировки остатка при
расчете процентов, если в параметре процентов задана сложная капитализация
макросом.
Пример.
record DEP_ACC_REC("depositr.dbt");
PC_ACC_REC
PC_ACC_REC – определитель типа RECORD записи таблицы параметров расчета
процентов. Значение переменной передается в макрос определения суммы
корректировки остатка при расчете процентов, если в параметре процентов
задана сложная капитализация макросом.
Пример.
record PC_ACC_REC ("pc_alg.dbt");
PrimaryDataInputMode
PrimaryDataInputMode – признак режима первоначального ввода договора.
RtCurDate
RtCurDate – дата операционного дня в RS-Retail.
RtMsgFile
RtMsgFile – имя таблицы системных сообщений RS-Retail.
RtRCurDate
RtRCurDate – значение переменной Rcurdate ИБС RS-Bank в RS-Retail.
RT_SpecialAccess_Client
RT_SpecialAccess_Client – признак разрешения отображать для пользователя записи о
клиентах, к которым требуется право специального доступа.
Глобальные переменные
SavedCurDate
SavedCurDate – сохраненная дата операционного дня.
SavedRCurDate
SavedRCurDate – значение переменной Rcurdate ИБС RS-Bank в RS-Retail.
между торговыми организациями, использующими для расчетов с клиентами пластиковые
карты, и Процессинговым центром. Модуль доступен в контексте любой подсистемы АС RS-
Retail V.6.
Классы
